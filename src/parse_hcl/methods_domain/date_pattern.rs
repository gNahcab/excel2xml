...
use crate::parse_hcl::methods_domain::date_bricks::{DateBricks, DateInfo, DateName, WrapperDateBricks};
...
impl DatePattern {
    ...
    fn is_consistent(&self) -> Result<(), HCLDataError> {
        if self.date.year.is_none() {
            return Err(HCLDataError::ParsingError(format!("the 'year' of 'date' doesn't exist in pattern-nr '{:?}', this is forbidden.", self.nr)));
        }
        if self.date.day.is_some() && self.date.year.is_some() && self.date.month.is_none() {
            return Err(HCLDataError::ParsingError(format!("the 'day' and 'year' of 'date' exist but not 'month' in pattern-nr '{:?}', this is forbidden: either day, month and year exist or month and year or year alone.", self.nr)));
        }
        if self.first_date.is_some() {
             if self.first_date.as_ref().unwrap().year.is_some() && self.first_date.as_ref().unwrap().month.is_none() {
                return Err(HCLDataError::ParsingError(format!("the 'day' and 'year' of 'first' exist but not 'month' in pattern-nr '{:?}', this is forbidden: either day, month and year exist or month and year or year or month or day alone.", self.nr)));
            }
            // if day exists: does day exist in date?
            if self.first_date.as_ref().unwrap().day.is_some() && self.date.day.is_none() {
                return Err(HCLDataError::ParsingError(format!("'day' exists in 'first' but not in 'date' in pattern-nr '{:?}', this is forbidden.", self.nr)));
            }
            // if month exists: does month exist in date?
            if self.first_date.as_ref().unwrap().month.is_some() && self.date.month.is_none() {
                return Err(HCLDataError::ParsingError(format!("'month' exists in 'first' but not in 'date' in pattern-nr '{:?}', this is forbidden.", self.nr)));
            }
        }
        Ok(())
    }
    pub fn to_regex(&self) -> Result<Regex, HCLDataError> {
        let regex_pattern= self.build_regex_pattern()?;
        let regex = Regex::new(regex_pattern.as_str())?;
        Ok(regex)
    }
    fn build_regex_pattern(&self) -> Result<String,HCLDataError> {
...
impl TransientDatePattern {
    ...
    pub(crate) fn add_nr(&mut self, labels: &Vec<BlockLabel>) -> Result<(), HCLDataError> {
        if labels.len() == 0 {
            return Err(HCLDataError::ParsingError(format!("a 'to_date'-pattern should have one label, but found a pattern that has zero.")))
        }
        if labels.len() > 1 {
            return Err(HCLDataError::ParsingError(format!("a pattern should only have one label, but has multiple '{:?}'", labels)))
        }
        ...
        let nr_ = match nr_ {
            Ok(number) => {number}
            Err(_) => {
                return Err(HCLDataError::ParsingError(format!("couldn't parse label of to-date-pattern to usize, is this a number: '{:?}'?",label)));
            }
        };
        self.nr = nr_;
        Ok(())
    }
    pub(crate) fn add_first(&mut self, first: &Body) -> Result<(), HCLDataError> {
        let date = WrapperDateBricks(first.to_owned()).to_date_bricks()?;
        self.first_date = Option::from(date);
        Ok(())
    }
    pub(crate) fn add_date(&mut self, date: &Body) -> Result<(), HCLDataError> {
...
impl WrapperDatePattern {
    pub(crate) fn to_pattern(&self) -> Result<DatePattern, HCLDataError> {
        ...
        for block in self.0.blocks() {
            match block.identifier.as_str() {
                ...
                _ => return Err(HCLDataError::ParsingError(format!("unknown block in 'to_date'-pattern: '{:?}'", block.identifier)))
            }
        }
